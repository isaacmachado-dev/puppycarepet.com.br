...
export const CartButton: React.FC<CartButtonProps> = ({ label, nextHref, ...props }) => {
  const router = useRouter();
  const pathname = typeof window !== "undefined" ? window.location.pathname : "";
  const isObrigado = pathname.includes("/cart/obrigado");
  ...
  return (
    ...
    >
      ...
      >
        <circle cx="24" cy="24" r="21" fill="#FFFFFF" fillOpacity="0" stroke="#FFFFFF" strokeWidth="3" />
        <path d="M18 15 L32 24 L18 33" stroke="#FFFFFF" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none" />
      </svg>
    </button>
  );
};
